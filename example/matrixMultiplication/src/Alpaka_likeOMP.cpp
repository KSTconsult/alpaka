...
public:
		...
		-> void
	{
		...
		// Elemnts per thread (How mmany rows multi one thread)
		...
		//cout << "Index" << gridThreadIdx  << gridThreadIdy <<" "  <<  beginRow << endRow << " "<< beginСol << endCols << endl;

		for (unsigned int i = beginRow; (i < endRow) && (i < numRows); i++)
		{
			for (unsigned int k = 0; k < N; k++)
			{
				// compute i-th element of result
				const double AA = A[i * numCols + k];
				const double* const BB = &B[k * numCols];
				double* const CC = &C[i * numCols];
...
-> int
{
	...
	Size const numElements(static_cast<double>(N));

	/*Create WorkDiv*/
	alpaka::vec::Vec<Dim, Size> const elementsPerThread(
		numElements / static_cast<Size>(num_threads));

	alpaka::vec::Vec<Dim, Size> const threadsPerBlock(
		static_cast<Size>(1));

	alpaka::vec::Vec<Dim, Size> const blocksPerGrid(
		numElements / (numElements / static_cast<Size>(num_threads)));
